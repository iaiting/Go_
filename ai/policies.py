...
class Go_NN_policy():
    ...
    def __init__(self, nn_weights_file_path):
        self.neural_network = NeuralNetwork([(9 * 9 * 3) , 80, 50, 80, 9 * 9])
        ...
        while not passed_twice:  # game ends when two sides pass
          ...
          #  if(count % 50 == 0):
            if move is not None:
                ...
                outputs = rollout_policy.neural_network.get_outpus()
                # print outputs[move[0]*9+move[1]]
                # print "6,4:"+str(outputs[6*9+4]) + "," + str(rollout_policy.legal_move(game_state, [6,4]))
                # print "8,8:"+str(outputs[8*9+8]) + "," + str(rollout_policy.legal_move(game_state, [8,8]))
                # if count == 67 or count == 68:
                #     print game_state.is_move_ko([8,8],game_state.turn)
                #     game_state.printMe()
            #game_state.printMe()
            print ""
...
#TEST
#print Go_Simulator().simulate(GoGame_Logic(9), 1, Go_NN_rollout_policy('nn_weights.txt'))
